...
# New checkbox for "Add Watermark" defaults to OFF, optional.
...
class ImageProcessorApp:
    def __init__(self, root):
        ...
        self.root.geometry("175x750")  # Taller for the new watermark checkbox
        ...
        # Watermark path
        self.watermark_path = r"C:\PythonProjects\Axly.comFiles\Utils\CombinedProcessor\axlywm.png"
    ...
    def create_ui(self):
        ...
        tk.Checkbutton(self.root, text="Add Watermark?", variable=self.watermark_var).pack()
    ...
    def on_drop(self, event):
        ...
        for file_path in files:
            self.process_image(file_path, genre, subgenre, desc, tag, folder, remove_meta, add_meta, add_watermark)
            ...
            if not remove_meta and not add_meta and not add_watermark:
                ...
                with Image.open(file_path).convert("RGBA") as img:
                    if remove_meta:
                        ...
                        # Copy original to new_path without metadata change
                        img.save(new_path, quality=95)  # JPG quality 95, PNG lossless

                    if add_meta:
                        # Add metadata to new_path
                        self.add_metadata(new_path, ext_lower)

                    if add_watermark and os.path.exists(self.watermark_path):
                        # Add watermark
                        watermark = Image.open(self.watermark_path).convert("RGBA")
                        # Resize watermark to 10% of base image width
                        scale_factor = 0.1
                        ...
                        # Apply 30% opacity
                        watermark.putalpha(int(255 * 0.3))
                        # Position: bottom-right with 10px padding
                        x = img.width - watermark.width - 10
                        y = img.height - watermark.height - 10
                        # Paste watermark
                        img.paste(watermark, (x, y), watermark)
                        img.save(new_path, quality=95)  # Save with watermark

                # Original file remains; delete it to mimic move
                os.remove(file_path)
    ...
    def strip_metadata(self, img, output_path, ext):
        ...
        if ext == '.png':
            # For PNG: Save without info (lossless, but re-compresses)
            img.save(output_path)
        elif ext in ('.jpg', '.jpeg'):
            # For JPG: Remove EXIF without re-encoding
            img.save(output_path, quality=95, exif=None)
